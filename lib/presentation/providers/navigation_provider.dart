...
/// 항행이력 상태 관리 Provider - 영구 캐시 지원 버전
...
//  List<dynamic> get rosList => _rosList;
  ...
  Future<void> getRosList({
    ...
    // 1️⃣ 메모리 캐시 확인
    final memoryCached = _cache.get<List<dynamic>>(cacheKey);
    ...
    // 2️⃣ 영구 저장소 캐시 확인 (오프라인 지원)
    try {
      final persistentCached = await CacheManager.getCache(cacheKey);
      if (persistentCached != null) {
        AppLogger.d('✅ [영구 캐시 사용] 항행이력 복원');

        // List<dynamic>으로 캐스팅
        _rosList = persistentCached as List<dynamic>;

        // 메모리 캐시에도 복사
        _cache.put(cacheKey, _rosList, const Duration(hours: 1));

        AppLogger.d('📊 영구 캐시 데이터: ${_rosList.length}건');
        notifyListeners();
        return;
      }
    } catch (e) {
